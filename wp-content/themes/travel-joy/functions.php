...
if (!function_exists('travel_joy_is_wptravel_related_page')) {
	...
	function travel_joy_is_wptravel_related_page($page_type = false)
	{
		if (travel_joy_is_wp_travel_active()) {
			if ($page_type) {
				if (travel_joy_itinerary_is_cart_page()) {
					return 'wp-travel-cart-page';
				} elseif (travel_joy_itinerary_is_checkout_page()) {
					return 'wp-travel-checkout-page';
				} elseif (travel_joy_itinerary_is_dashboard_page()) {
					return 'wp-travel-dashboard-page';
				} else {
					return false;
				}
			} else {
				return (travel_joy_itinerary_is_cart_page() || travel_joy_itinerary_is_checkout_page() || travel_joy_itinerary_is_dashboard_page()) ? true : false;
			}
		} else {
			return false;
		}
