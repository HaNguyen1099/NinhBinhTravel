			...
			<form class="description-box" action="">
				...
				<div class="item item-5">
						...
						$args     = array(
							...
							'show_option_none'  => esc_html__('Activity', 'travel-joy'),
							'option_none_value' => esc_html__('Activity', 'travel-joy'),
							'hide_empty'      => 0,
							'selected'        => 1,
							'hierarchical'    => 1,
							'name'            => $taxonomy_name,
			...
			<section class="grid-box">
				<ul>
					...
					foreach ($trips_dropdown as $trip_id) {
						...
						$price_per_text = travel_joy_itinerary_get_price_per_text($trip_id);
						$trip_price     = travel_joy_itinerary_get_price($trip_id);
						$regular_price  = travel_joy_itinerary_get_price($trip_id, true);
						$enable_sale    = travel_joy_itinerary_is_sale_enabled($trip_id, true);
						$sale_price     = travel_joy_itinerary_get_price($trip_id);

						// Departures and durations.
						$trip_dates          = travel_joy_itinerary_get_trip_dates($trip_id);
						$trip_duration       = !empty($trip_dates['trip_duration']) ? $trip_dates['trip_duration'] : '';
						$trip_duration_night = !empty($trip_dates['trip_duration_night']) ? $trip_dates['trip_duration_night'] : '';
						// $duration            = !empty($trip_dates['duration']) ? $trip_dates['duration'] : '';
						$trip_start_dates    = !empty($trip_dates['trip_start_dates']) ? $trip_dates['trip_start_dates'] : array();


						// check if fixed departure is enabled
						$trip_duration_check_fixed_departure  = WpTravel_Helpers_Trips::get_trip($trip_id)['trip']['is_fixed_departure'];
						if ($trip_duration_check_fixed_departure) {
							$duration  = WpTravel_Helpers_Trips::get_trip($trip_id)['trip']['display_dates'];
						} else {
							$trip_duration_filtered = wp_travel_get_trip_durations($trip_id);
							$duration = $trip_duration_filtered;
						}
						...
						// var_dump( WpTravel_Helpers_Trips::get_trip($trip_id)['trip']); die(); 
							...
							<figure>
								<div class="image-top">
									<a href="<?php echo esc_url($trip_url); ?>">
										<?php echo travel_joy_itinerary_get_post_thumbnail($trip_id); // phpcs:ignore 
										?>
									</a>
									<div class="overlay">
										<div class="image-overlay overlay__bottom">
											<div class="left">
												<?php if (!empty($trip_price)) { ?>
													<?php if ($enable_sale && !empty($sale_price)) { ?>
														<h2>
															<del>
																<?php echo travel_joy_itinerary_get_formated_price_currency($regular_price, true, '', $trip_id); // phpcs:ignore 
																?>
															</del>
															<h1><?php echo travel_joy_itinerary_get_formated_price_currency($sale_price, false, '', $trip_id) . sprintf(__('%s', 'travel-joy'), $price_per_text); // phpcs:ignore 
																?></h1>
														</h2>
													<?php } else { ?>
														<h1>
															<?php echo travel_joy_itinerary_get_formated_price_currency($trip_price, true, '', $trip_id) . sprintf(__('%s', 'travel-joy'), $price_per_text); // phpcs:ignore 
															?>
														</h1>
													<?php } ?>
												<?php } else { ?>
								...
								<div class="other-info">
									<div class="title">
										...
										<div class="address-time">
											...
											<?php
											if ((isset($duration) && !count($trip_start_dates) > 0) && ($trip_duration || $trip_duration_night)) {
											?>
												<span><i class="fas fa-clock"></i>
													<p><?php echo wp_kses_post($duration); ?></p>
												</span>
											<?php
											} else {
											?>
												<span><i class="fas fa-clock"></i>
													<p><?php echo wp_kses_post("Not available"); ?></p>
												</span>
											<?php
											}
											?>
