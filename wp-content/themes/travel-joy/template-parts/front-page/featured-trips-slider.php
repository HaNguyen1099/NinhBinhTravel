		...
		<div class="section-wrapper">
			<div class="slideshow-container">
				<section class="lazy slider" data-sizes="">
					...
					if ( is_array( $trips ) && count( $trips ) > 0 ) {
						foreach ( $trips as $trip ) {
							$settings    = travel_joy_itinerary_get_settings();
							$trip_id     = ! empty( $trip->ID ) ? $trip->ID : '';
							$is_featured = ! empty( $trip_id ) ? get_post_meta( $trip_id, 'wp_travel_featured', true ) : false;
							if ( 'yes' === $is_featured ) {
								...
								$average_rating   = travel_joy_itinerary_get_average_rating( $trip_id );
								...
								$count_review	 = (int) get_comments_number($trip_id);
								...
								$currency_symbol = travel_joy_itinerary_get_currency_symbol( $currency_code );
								...
								$min_pricing_id  = travel_joy_itinerary_get_min_pricing_id( $trip_id );
									...
									<div class="flex-box">
										<div class="flex__image-box box__shadow">
											<a href="<?php echo esc_url( $trip_url ); ?>">
												<?php echo travel_joy_itinerary_get_post_thumbnail( $trip_id, 'large' ); // phpcs:ignore ?>
											</a>
										</div>
										<div class="flex__description-box">
											...
											<div class="review-box">
												<?php if ( ! empty( $trip_price ) ) { ?>
													<?php if ( $enable_sale && ! empty( $sale_price ) ) { ?>
														<p>
															...
															<?php echo travel_joy_itinerary_get_formated_price_currency( $sale_price, false, '', $trip_id ); // phpcs:ignore ?>
														</p>
													<?php } else { ?>
														<p>
															<?php echo travel_joy_itinerary_get_formated_price_currency( $trip_price, true, '', $trip_id  ); // phpcs:ignore ?>
														</p>
													<?php } ?>
												<?php } else { ?>
												...
												<div class="review-item__right" title="<?php printf( esc_attr__( 'Rated %s out of 5', 'travel-joy' ), $average_rating ); // phpcs:ignore ?>">
													...
													<p><?php printf( esc_html__( '%d reviews', 'travel-joy' ), $count_review ); // phpcs:ignore ?></p>
												</div>
											</div>
